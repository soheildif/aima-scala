...
object SentenceToCNF {
  ...
  def standardizeQuantifierVariables(s: Sentence) =
    s match {
      ...
      case x: Negation =>
        x.sentence match {
          ...
          case _ => //negationsIn happened before, this should not happen
            throw new IllegalStateException("Negation of non AtomicSentence in standardizeQuantifierVariables")
        }
      case x: Conjunction =>
      ...
      case x: UniversalQuantifier =>
        val newVar = StandardizeVariable(x.variable) //TODO
        new UniversalQuantifer(newVar, 
                               standardizeQuantifierVariables(Subst(Map(x.variable -> newVar),x.sentence)))
      case x: ExistentialQuantifier =>
